...
export module PonyEngine.Render.PixelHandlers:UintPixelHandler;
...
export namespace PonyEngine::Render::PixelHandlers
{
	...
	class UintPixelHandler final : public IPixelHandler
	{
	public:
		...
		UintPixelHandler() noexcept = default;
		UintPixelHandler(const UintPixelHandler&) = delete;
		UintPixelHandler(UintPixelHandler&&) = delete;
		...
		UintPixelHandler& operator =(const UintPixelHandler&) = delete;
		UintPixelHandler& operator =(UintPixelHandler&&) = delete;
	};
}
...
namespace PonyEngine::Render::PixelHandlers
{
	template <std::uint64_t RedOffset, std::uint64_t GreenOffset, std::uint64_t BlueOffset, std::uint64_t AlphaOffset,
		std::uint64_t RedBits, std::uint64_t GreenBits, std::uint64_t BlueBits, std::uint64_t AlphaBits>
	PonyMath::Color::RGBA<float> UintPixelHandler<RedOffset, GreenOffset, BlueOffset, AlphaOffset,
			...
			const PonyMath::Core::Vector3<std::uint32_t>& pixelCoordinate) const
	{
		...
		{
			color.R() = UintToFloat<RedOffset, RedBits>(span);
		}
		if constexpr (GreenBits > 0)
		{
			color.G() = UintToFloat<GreenOffset, GreenBits>(span);
		}
		if constexpr (BlueBits > 0)
		{
			color.B() = UintToFloat<BlueOffset, BlueBits>(span);
		}
		if constexpr (AlphaBits > 0)
		{
			color.A() = UintToFloat<AlphaOffset, AlphaBits>(span);
		}
	...
	template <std::uint64_t RedOffset, std::uint64_t GreenOffset, std::uint64_t BlueOffset, std::uint64_t AlphaOffset,
		std::uint64_t RedBits, std::uint64_t GreenBits, std::uint64_t BlueBits, std::uint64_t AlphaBits>
	void UintPixelHandler<RedOffset, GreenOffset, BlueOffset, AlphaOffset,
			...
			const PonyMath::Core::Vector3<std::uint32_t>& pixelCoordinate, const PonyMath::Color::RGBA<float>& color) const
	{
		...
		if constexpr (RedBits > 0)
		{
			FloatToUint<RedOffset, RedBits>(span, color.R());
		}
		if constexpr (GreenBits > 0)
		{
			FloatToUint<GreenOffset, GreenBits>(span, color.G());
		}
		if constexpr (BlueBits > 0)
		{
			FloatToUint<BlueOffset, BlueBits>(span, color.B());
		}
		if constexpr (AlphaBits > 0)
		{
			FloatToUint<AlphaOffset, AlphaBits>(span, color.A());
		}
	}
}
