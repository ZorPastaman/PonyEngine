...
export namespace PonyMath::Core
{
	...
	public:
		...
		[[nodiscard("Pure constructor")]]
		explicit constexpr Vector3(std::span<const T, ComponentCount> span) noexcept;
		[[nodiscard("Pure constructor")]]
		...
		/// @brief Gets the vector span. The order is x, y, z.
		/// @return Span.
		[[nodiscard("Pure function")]]
		...
		[[nodiscard("Pure function")]]
		Vector3 Normalized(const Vector3& fallback) const noexcept requires (std::is_floating_point_v<T>);
		/// @brief Normalizes the vector.
		/// @param fallback Fallback vector. This vector is set if the magnitude of the vector is 0.
		void Normalize(const Vector3& fallback) noexcept requires (std::is_floating_point_v<T>);
		...
		/// @brief Swaps the components and returns a vector in order z, y, x.
		/// @return Swapped vector.
		[[nodiscard("Pure function")]]
		constexpr Vector3 Swapped() const noexcept;
		/// @brief Swaps the components in the vector. The order is z, y, x.
	...
	/// @brief Computes a cross product of the two vectors.
	/// @tparam T Component type.
	/// @param left Left vector.
	/// @param right Right vector.
	...
	/// @return Distance.
	template<Arithmetic T> [[nodiscard("Pure function")]]
	typename Vector3<T>::ComputationalType Distance(const Vector3<T>& left, const Vector3<T>& right) noexcept;
	...
	/// @return Distance
	template<Arithmetic T> [[nodiscard("Pure function")]]
	constexpr T DistanceSquared(const Vector3<T>& left, const Vector3<T>& right) noexcept;
	...
	/// @return Clamped vector.
	template<std::floating_point T> [[nodiscard("Pure function")]]
	Vector3<T> ClampMagnitude(const Vector3<T>& vector, T magnitude) noexcept;
...
namespace PonyMath::Core
{
	...
	template<Arithmetic T>
	constexpr Vector3<T>::Vector3(const std::span<const T, ComponentCount> span) noexcept
	{
	...
	template<Arithmetic T>
	constexpr std::span<T, 3> Vector3<T>::Span() noexcept
	{
		return components;
	}

	template<Arithmetic T>
	constexpr std::span<const T, 3> Vector3<T>::Span() const noexcept
	{
		return components;
	}
	...
	template <Arithmetic T>
	Vector3<T> Vector3<T>::Normalized(const Vector3& fallback) const noexcept requires (std::is_floating_point_v<T>)
	{
		return IsAlmostZero() ? fallback : Normalized();
	}

	template <Arithmetic T>
	void Vector3<T>::Normalize(const Vector3& fallback) noexcept requires (std::is_floating_point_v<T>)
	{
	...
	typename Vector3<T>::ComputationalType Distance(const Vector3<T>& left, const Vector3<T>& right) noexcept
	{
		return std::sqrt(static_cast<typename Vector3<T>::ComputationalType>(DistanceSquared(left, right)));
	}
	...
	template <std::floating_point T>
	Vector3<T> ClampMagnitude(const Vector3<T>& vector, const T magnitude) noexcept
	{
