...
export namespace PonyMath::Core
{
	...
	public:
		...
		[[nodiscard("Pure operator")]]
		constexpr bool operator ==(const Vector4& other) const noexcept = default;
	...
	/// @return Distance.
	template<Arithmetic T> [[nodiscard("Pure function")]]
	typename Vector4<T>::ComputationalType Distance(const Vector4<T>& left, const Vector4<T>& right) noexcept;
	...
	/// @return Distance
	template<Arithmetic T> [[nodiscard("Pure function")]]
	constexpr T DistanceSquared(const Vector4<T>& left, const Vector4<T>& right) noexcept;
	...
	template<Arithmetic T> [[nodiscard("Pure function")]]
	Vector4<T> Abs(const Vector4<T>& vector) noexcept;
	...
	template<Arithmetic T> [[nodiscard("Pure function")]]
	constexpr Vector4<T> Min(const Vector4<T>& left, const Vector4<T>& right) noexcept;
	...
	template<Arithmetic T> [[nodiscard("Pure function")]]
	constexpr Vector4<T> Max(const Vector4<T>& left, const Vector4<T>& right) noexcept;
	...
	template<Arithmetic T> [[nodiscard("Pure function")]]
	constexpr Vector4<T> Clamp(const Vector4<T>& value, const Vector4<T>& min, const Vector4<T>& max) noexcept;
	...
	/// @return Clamped vector.
	template<std::floating_point T> [[nodiscard("Pure function")]]
	Vector4<T> ClampMagnitude(const Vector4<T>& vector, T magnitude) noexcept;
...
namespace PonyMath::Core
{
	...
	template<Arithmetic T>
	constexpr std::pair<T&, T&> Vector4<T>::MinMax() noexcept
	{
	...
	template<Arithmetic T>
	constexpr std::pair<const T&, const T&> Vector4<T>::MinMax() const noexcept
	{
		auto [min, max] = std::ranges::minmax_element(components);
	...
	typename Vector4<T>::ComputationalType Distance(const Vector4<T>& left, const Vector4<T>& right) noexcept
	{
		return std::sqrt(static_cast<typename Vector4<T>::ComputationalType>(DistanceSquared(left, right)));
	}
	...
	template <std::floating_point T>
	Vector4<T> ClampMagnitude(const Vector4<T>& vector, const T magnitude) noexcept
	{
