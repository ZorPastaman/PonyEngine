...
export namespace PonyMath::Geometry
{
	...
	public:
		...
		/// @return Vertex color.
		[[nodiscard("Pure function")]]
		const Color::RGBA<float>& Color(std::uint32_t vertexIndex) const noexcept;
		/// @brief Sets a vertex color by the index.
		/// @param vertexIndex Vertex index.
		/// @param color Vertex color to set.
		void Color(std::uint32_t vertexIndex, const Color::RGBA<float>& color) noexcept;
		/// @brief Gets vertex colors.
		/// @return Vertex colors.
		[[nodiscard("Pure function")]]
		std::span<const Color::RGBA<float>> Colors() const noexcept;
		...
		void Colors(std::span<const Color::RGBA<float>> colorsToSet);
	...
	private:
		...
		std::vector<Color::RGBA<float>> colors; ///< Vertex colors.
	};
}
...
namespace PonyMath::Geometry
{
	...
	void Mesh::VertexCount(const std::uint32_t vertexCount)
	{
		...
		auto newColors = std::vector<Color::RGBA<float>>(HasColors() ? vertexCount : 0);
	...
	const Color::RGBA<float>& Mesh::Color(const std::uint32_t vertexIndex) const noexcept
	{
		return colors[vertexIndex];
	}

	void Mesh::Color(const std::uint32_t vertexIndex, const Color::RGBA<float>& color) noexcept
	{
		colors[vertexIndex] = color;
	}

	std::span<const Color::RGBA<float>> Mesh::Colors() const noexcept
	{
		return colors;
	}

	void Mesh::Colors(const std::span<const Color::RGBA<float>> colorsToSet)
	{
