...
export namespace PonyMath::Geometry
{
	...
	{
	public:
		using VertexType = Core::Vector3<float>;
		using TriangleType = Core::Vector3<std::uint32_t>;
		using VertexColorType = Core::RGBA<float>;
	...
	private:
		std::vector<Core::Vector3<float>> vertices;
		std::vector<Core::Vector3<std::uint32_t>> triangles;

		std::vector<Core::RGBA<float>> colors;
	};
}
...
namespace PonyMath::Geometry
{
	...
	void Mesh::VertexCount(const std::uint32_t vertexCount)
	{
		...
		auto newColors = std::vector<Core::RGBA<float>>(colors.size() > 0 ? vertexCount : colors.size());
	...
	void Mesh::Vertices(const std::span<const Core::Vector3<float>> verticesToSet)
	{
	...
	void Mesh::Triangles(const std::span<const Core::Vector3<std::uint32_t>> trianglesToSet)
	{
		...
		const std::uint32_t* const end = current + trianglesToSet.size() * Core::Vector3<std::uint32_t>::ComponentCount;
