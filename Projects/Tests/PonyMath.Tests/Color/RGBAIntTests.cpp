...
namespace Color
{
	TEST_CLASS(RGBAIntTests)
	{
		...
		{
			Assert::IsTrue(std::is_same_v<std::uint8_t, PonyMath::Color::RGBAInt<std::uint8_t>::ValueType>);
		}

		TEST_METHOD(StaticDataTest)
		{
			Assert::AreEqual(std::numeric_limits<std::uint8_t>::max(), PonyMath::Color::RGBAInt<std::uint8_t>::MaxValue);
			Assert::AreEqual(std::size_t{4}, PonyMath::Color::RGBAInt<std::uint8_t>::ComponentCount);
		}

		TEST_METHOD(PredefinedTest)
		{
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Red == PonyMath::Color::RGBAInt<std::uint8_t>(255, 0, 0, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Green == PonyMath::Color::RGBAInt<std::uint8_t>(0, 255, 0, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Blue == PonyMath::Color::RGBAInt<std::uint8_t>(0, 0, 255, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Black == PonyMath::Color::RGBAInt<std::uint8_t>(0, 0, 0, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::White == PonyMath::Color::RGBAInt<std::uint8_t>(255, 255, 255, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Clear == PonyMath::Color::RGBAInt<std::uint8_t>(0, 0, 0, 0));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Yellow == PonyMath::Color::RGBAInt<std::uint8_t>(255, 255, 0, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Magenta == PonyMath::Color::RGBAInt<std::uint8_t>(255, 0, 255, 255));
			Assert::IsTrue(PonyMath::Color::RGBAInt<std::uint8_t>::Predefined::Cyan == PonyMath::Color::RGBAInt<std::uint8_t>(0, 255, 255, 255));
		}

		TEST_METHOD(ConstructorTest)
		{
			auto defaultColor = PonyMath::Color::RGBAInt<std::uint8_t>();
			...
			const auto arrayColor = PonyMath::Color::RGBAInt<std::uint8_t>(array);
			Assert::AreEqual(r, arrayColor.R());
			Assert::AreEqual(g, arrayColor.G());
			Assert::AreEqual(b, arrayColor.B());
			Assert::AreEqual(a, arrayColor.A());

			constexpr auto rgb = PonyMath::Color::RGBInt(r, g, b);
			const auto rgba = PonyMath::Color::RGBAInt<std::uint8_t>(rgb, a);
			Assert::AreEqual(r, rgba.R());
			Assert::AreEqual(g, rgba.G());
			Assert::AreEqual(b, rgba.B());
			Assert::AreEqual(a, rgba.A());

			constexpr auto vector = PonyMath::Core::Vector4<std::uint8_t>(r, g, b, a);
			auto vectorColor = PonyMath::Color::RGBAInt<std::uint8_t>(vector);
		...
		TEST_METHOD(IsBlackTest)
		{
			...
			constexpr std::uint8_t b = 1;
			constexpr std::uint8_t a = 201;
			constexpr auto color = PonyMath::Color::RGBAInt<std::uint8_t>(r, g, b, a);
		...
		TEST_METHOD(MinCombinedTest)
		{
			...
			constexpr auto color1 = PonyMath::Color::RGBAInt<std::uint8_t>(r1, g1, b1, a1);
		...
		TEST_METHOD(ClampTest)
		{
			...
			constexpr auto color2 = PonyMath::Color::RGBAInt<std::uint8_t>(r2, g2, b2, a2);
		...
		TEST_METHOD(ToRGBIntTest)
		{
			...
			constexpr std::uint8_t a = 201;
			constexpr auto color = PonyMath::Color::RGBAInt<std::uint8_t>(r, g, b, a);
			const auto rgb = static_cast<PonyMath::Color::RGBInt<std::uint8_t>>(color);
		...
		TEST_METHOD(ToVectorTest)
		{
			...
			constexpr std::uint8_t a = 201;
			constexpr auto color = PonyMath::Color::RGBAInt<std::uint8_t>(r, g, b, a);
			const auto vector = static_cast<PonyMath::Core::Vector4<std::uint8_t>>(color);
		...
		TEST_METHOD(AssignmentOperatorsTest)
		{
			...
			auto otherColor = PonyMath::Color::RGBAInt<std::uint8_t>(otherR, otherG, otherB, otherA);
			PonyMath::Color::RGBAInt<std::uint8_t>& result = color = otherColor;
			...
			color = PonyMath::Color::RGBAInt<std::uint8_t>(r, g, b, a);
			PonyMath::Color::RGBAInt<std::uint8_t>& otherResult = otherColor = std::move(color);
		...
		TEST_METHOD(EqualTest)
		{
			...
			for (std::size_t i = 0; i < PonyMath::Color::RGBAInt<std::uint8_t>::ComponentCount; ++i)
			{
		...
		static constexpr PonyMath::Color::RGBAInt<std::uint8_t> ColorConstexpr()
		{
			auto color = PonyMath::Color::RGBAInt<std::uint8_t>(3, 250, 128, 111);
			auto copiedColor = color;
			auto movedColor = std::move(color);
		...
		{
			[[maybe_unused]] constexpr auto defaultColor = PonyMath::Color::RGBAInt<std::uint8_t>();
			[[maybe_unused]] constexpr auto color = PonyMath::Color::RGBAInt<std::uint8_t>(3, 250, 128, 100);
			[[maybe_unused]] constexpr auto rgb = PonyMath::Color::RGBAInt<std::uint8_t>(PonyMath::Color::RGBInt<std::uint8_t>(3, 156, 69), 100);
			[[maybe_unused]] constexpr auto arrayColor = PonyMath::Color::RGBAInt<std::uint8_t>(std::array{color.R(), color.G(), color.B(), color.A()});
			...
			[[maybe_unused]] constexpr auto minCombined = PonyMath::Color::Min(defaultColor, arrayColor);
			[[maybe_unused]] constexpr auto maxCombined = PonyMath::Color::Max(defaultColor, arrayColor);
			[[maybe_unused]] constexpr auto clamped = PonyMath::Color::Clamp(color, defaultColor, arrayColor);
		}
	};
}
