...
namespace Math
{
	TEST_CLASS(RGBATests)
	{
		...
		{
			Assert::IsTrue(std::is_same_v<float, PonyMath::Core::RGBA<float>::ValueType>);
		}

		TEST_METHOD(StaticDataTest)
		{
			Assert::AreEqual(std::size_t{4}, PonyMath::Core::RGBA<float>::ComponentCount);
		}

		TEST_METHOD(PredefinedTest)
		{
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Red == PonyMath::Core::RGBA<float>(1.f, 0.f, 0.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Green == PonyMath::Core::RGBA<float>(0.f, 1.f, 0.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Blue == PonyMath::Core::RGBA<float>(0.f, 0.f, 1.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Black == PonyMath::Core::RGBA<float>(0.f, 0.f, 0.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::White == PonyMath::Core::RGBA<float>(1.f, 1.f, 1.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Clear == PonyMath::Core::RGBA<float>(0.f, 0.f, 0.f, 0.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Gray == PonyMath::Core::RGBA<float>(0.5f, 0.5f, 0.5f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Yellow == PonyMath::Core::RGBA<float>(1.f, 1.f, 0.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Magenta == PonyMath::Core::RGBA<float>(1.f, 0.f, 1.f, 1.f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Cyan == PonyMath::Core::RGBA<float>(0.f, 1.f, 1.f, 1.f));
		}

		TEST_METHOD(ConstructorTest)
		{
			auto defaultColor = PonyMath::Core::RGBA<float>();
			...
			auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			...
			const auto arrayColor = PonyMath::Core::RGBA<float>(array);
			Assert::AreEqual(r, arrayColor.R());
			Assert::AreEqual(g, arrayColor.G());
			Assert::AreEqual(b, arrayColor.B());
			Assert::AreEqual(a, arrayColor.A());

			constexpr auto rgb = PonyMath::Core::RGB<float>(r, g, b);
			auto rgba = PonyMath::Core::RGBA<float>(rgb, a);
			...
			constexpr auto rgbaInt = PonyMath::Core::RGBAInt<std::uint8_t>(red, green, blue, alpha);
			auto convertedRGBA = PonyMath::Core::RGBA<float>(rgbaInt);
			...
			auto convertedRGB = PonyMath::Core::RGBA<float>(rgbInt, a);
			...
			auto vectorColor = PonyMath::Core::RGBA<float>(vector);
		...
		TEST_METHOD(AccessTest)
		{
			...
			Assert::AreEqual(b, color.B());
			Assert::AreEqual(a, color.A());
			constexpr auto cColor = PonyMath::Core::RGBA<float>(r, g, b, a);
		...
		TEST_METHOD(GrayscaleTest)
		{
			...
			constexpr float b = 0.98f;
			constexpr float a = 0.95f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
		...
		TEST_METHOD(IsBlackTest)
		{
			...
			Assert::IsTrue(PonyMath::Core::RGBAInt<std::uint8_t>::Predefined::Black.IsBlack());
		}

		TEST_METHOD(IsAlmostBlackTest)
		{
			auto color = PonyMath::Core::RGBA<float>::Predefined::Black;
			Assert::IsTrue(color.IsAlmostBlack());
			Assert::IsTrue(color.IsAlmostBlack(0.5f));

			for (std::size_t i = 0; i < PonyMath::Core::RGBA<float>::ComponentCount; ++i)
			{
				...
				Assert::IsTrue(color.IsAlmostBlack(0.5f));
				color[i] = PonyMath::Core::RGBA<float>::Predefined::Black[i];
			}
		}
		...
		TEST_METHOD(IsWhiteTest)
		{
			...
			Assert::IsTrue(PonyMath::Core::RGBAInt<std::uint8_t>::Predefined::White.IsWhite());
		}

		TEST_METHOD(IsAlmostWhiteTest)
		{
			auto color = PonyMath::Core::RGBA<float>::Predefined::White;
			...
			for (std::size_t i = 0; i < PonyMath::Core::RGBA<float>::ComponentCount; ++i)
			{
				...
				Assert::IsTrue(color.IsAlmostWhite(0.5f));
				color[i] = PonyMath::Core::RGBA<float>::Predefined::White[i];
			}
		}
		...
		TEST_METHOD(IsTransparent)
		{
			...
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Clear.IsTransparent());
		}
		...
		TEST_METHOD(IsAlmostTransparent)
		{
			...
			Assert::IsTrue(color.IsAlmostTransparent(0.5f));
			Assert::IsTrue(PonyMath::Core::RGBA<float>::Predefined::Clear.IsAlmostTransparent());
		}
		...
		TEST_METHOD(SetTest)
		{
			...
			constexpr float b = 0.211f;
			constexpr float a = 0.166f;
			auto color = PonyMath::Core::RGBA<float>();
			color.Set(r, g, b, a);
			Assert::AreEqual(r, color.R());
			Assert::AreEqual(g, color.G());
			Assert::AreEqual(b, color.B());
			...
			color = PonyMath::Core::RGBA<float>();
			color.Set(array);
			Assert::AreEqual(r, color.R());
			Assert::AreEqual(g, color.G());
			Assert::AreEqual(b, color.B());
		...
		TEST_METHOD(ToRGBATest)
		{
			...
			constexpr float a = 0.166f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			auto doubleColor = static_cast<PonyMath::Core::RGBA<double>>(color);
		...
		TEST_METHOD(ToRGBTest)
		{
			...
			constexpr float a = 0.166f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			auto rgb = static_cast<PonyMath::Core::RGB<float>>(color);
		...
		TEST_METHOD(ToRGBAIntTest)
		{
			...
			constexpr float a = 0.166f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			auto rgba = static_cast<PonyMath::Core::RGBAInt<std::uint8_t>>(color);
		...
		TEST_METHOD(ToVectorTest)
		{
			...
			constexpr float a = 0.166f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			auto vector = static_cast<PonyMath::Core::Vector4<float>>(color);
		...
		TEST_METHOD(ComponentAccessTest)
		{
			...
			constexpr auto colorC = PonyMath::Core::RGBA<float>(r, g, b, a);
		...
		TEST_METHOD(CopyAssignmentTest)
		{
			...
			constexpr float a = 0.166f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			auto copiedColor = PonyMath::Core::RGBA<float>();
			PonyMath::Core::RGBA<float>& colorRef = copiedColor = color;
			Assert::AreEqual(r, copiedColor.R());
			Assert::AreEqual(g, copiedColor.G());
			Assert::AreEqual(b, copiedColor.B());
			Assert::AreEqual(a, copiedColor.A());
		...
		TEST_METHOD(MoveAssignmentTest)
		{
			...
			constexpr float a = 0.166f;
			auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			auto movedColor = PonyMath::Core::RGBA<float>();
			PonyMath::Core::RGBA<float>& colorRef = movedColor = std::move(color);
			Assert::AreEqual(r, movedColor.R());
			Assert::AreEqual(g, movedColor.G());
			Assert::AreEqual(b, movedColor.B());
			Assert::AreEqual(a, movedColor.A());
		...
		TEST_METHOD(SumAssignmentTest)
		{
			...
			constexpr auto color1 = PonyMath::Core::RGBA<float>(r1, g1, b1, a1);
			PonyMath::Core::RGBA<float>& colorRef = color += color1;
		...
		TEST_METHOD(ProductAssignmentTest)
		{
			...
			PonyMath::Core::RGBA<float>& colorRef = color *= multiplier;
		...
		TEST_METHOD(DivisionAssignmentTest)
		{
			...
			PonyMath::Core::RGBA<float>& colorRef = color /= divisor;
		...
		TEST_METHOD(DistanceTest)
		{
			...
			constexpr float a1 = 0.366f;
			constexpr auto color1 = PonyMath::Core::RGBA<float>(r1, g1, b1, a1);
			Assert::AreEqual(0.49, static_cast<double>(PonyMath::Core::DistanceSquared(color, color1)), 0.001);
			Assert::AreEqual(0.7, static_cast<double>(PonyMath::Core::Distance(color, color1)), 0.001);
		}
		...
		TEST_METHOD(ClampTest)
		{
			...
			constexpr auto color2 = PonyMath::Core::RGBA<float>(r2, g2, b2, a2);
		...
		TEST_METHOD(LerpTest)
		{
			...
			PonyMath::Core::RGBA<float> lerped = PonyMath::Core::Lerp(color, color1, 0.f);
		...
		TEST_METHOD(AreAlmostEqualTest)
		{
			...
			constexpr float a = 0.166f;
			constexpr auto color = PonyMath::Core::RGBA<float>(r, g, b, a);
			PonyMath::Core::RGBA<float> colorL = color;
		...
		TEST_METHOD(ProductTest)
		{
			...
			product = PonyMath::Core::RGBA<float>();
		...
		static constexpr PonyMath::Core::RGBA<float> RGBAConstexpr()
		{
			auto color = PonyMath::Core::RGBA<float>(0.3f, 0.4f, 0.6f, 0.69f);
			...
			movedColor[0] *= 1.5f;
			auto anotherColor = PonyMath::Core::RGBA<float>();
		...
		{
			[[maybe_unused]] constexpr auto defaultColor = PonyMath::Core::RGBA<float>();
			[[maybe_unused]] constexpr auto color = PonyMath::Core::RGBA<float>(0.3f, 0.4f, 0.6f, 0.69f);
			[[maybe_unused]] constexpr auto spanColor = PonyMath::Core::RGBA<float>(std::array<float, 4>{0.4f, 0.1f, 0.2f, 0.9f});
			[[maybe_unused]] constexpr auto rgbColor = PonyMath::Core::RGBA<float>(PonyMath::Core::RGB<float>(0.1f, 0.2f, 0.3f), 0.4f);
			[[maybe_unused]] constexpr auto rgbIntColor = PonyMath::Core::RGBA<float>(PonyMath::Core::RGBInt<unsigned int>(54, 38, 90), 0.4f);
			[[maybe_unused]] constexpr auto rgbaColor = PonyMath::Core::RGBA<float>(PonyMath::Core::RGBAInt<unsigned short>(215, 574, 136, 907));
			[[maybe_unused]] constexpr auto vectorColor = PonyMath::Core::RGBA<float>(PonyMath::Core::Vector4<float>(0.4f, 0.69f, 0.228f, 0.322f));
			...
			[[maybe_unused]] constexpr auto rgba = static_cast<PonyMath::Core::RGBA<double>>(color);
			[[maybe_unused]] constexpr auto rgb = static_cast<PonyMath::Core::RGB<float>>(color);
			[[maybe_unused]] constexpr auto rgbaInt = static_cast<PonyMath::Core::RGBAInt<unsigned int>>(color);
