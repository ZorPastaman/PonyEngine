...
namespace Render
{
	TEST_CLASS(RenderSystemFactoryTests)
	{
		...
		TEST_METHOD(DefaultResolutionTest)
		{
			...
			Assert::IsTrue(dynamic_cast<PonyEngine::Window::IWindowSystem*>(std::get<1>(window.system).get())->Resolution() == dynamic_cast<PonyEngine::Render::IRenderSystem*>(renderSystem)->RenderTarget().Resolution());
		}
