...
namespace Math
{
	TEST_CLASS(QuaternionTests)
	{
		...
		{
			[[maybe_unused]] auto quaternionToMove = PonyEngine::Math::Quaternion<float>(0, 4, 5, 1);
			PonyEngine::Math::Quaternion<float> movedQuaternion = std::move(quaternionToMove);

			[[maybe_unused]] auto quaternion = PonyEngine::Math::Quaternion<float>(0, 4, 5, 1);
			quaternion.X() *= 3.f;
			quaternion.Y() /= 4.f;
			quaternion.Z() += 2.f;
			quaternion.W() -= 1.f;
		...
		{
			[[maybe_unused]] constexpr auto identity = PonyEngine::Math::Quaternion<float>::Predefined::Identity;

			[[maybe_unused]] constexpr auto defaultQuaternion = PonyEngine::Math::Quaternion<float>();
			[[maybe_unused]] constexpr auto quaternion = PonyEngine::Math::Quaternion<float>(0, 4, 5, 1);
			[[maybe_unused]] constexpr auto vectorQuaternion = PonyEngine::Math::Quaternion<float>(PonyEngine::Math::Vector4<float>(0, 4, 5, 1));
			[[maybe_unused]] constexpr PonyEngine::Math::Quaternion<float> copiedQuaternion = quaternion;
			[[maybe_unused]] constexpr PonyEngine::Math::Quaternion<float> movedQuaternion = QuaternionConstexpr();
			...
			[[maybe_unused]] constexpr PonyEngine::Math::Quaternion<float> conjugate = quaternion.Conjugate();
			[[maybe_unused]] constexpr PonyEngine::Math::Quaternion<float> inverse = quaternion.Inverse();
			...
			[[maybe_unused]] constexpr float dot = PonyEngine::Math::Dot(quaternion, vectorQuaternion);
			[[maybe_unused]] constexpr PonyEngine::Math::Quaternion<float> lerped = PonyEngine::Math::Lerp(defaultQuaternion, quaternion, 0.5f);

			[[maybe_unused]] constexpr PonyEngine::Math::Quaternion<float> product = quaternion * vectorQuaternion;
			[[maybe_unused]] constexpr PonyEngine::Math::Vector3<float> productV = quaternion * PonyEngine::Math::Vector3<float>(0, 4, 5);
		}
	};
}
