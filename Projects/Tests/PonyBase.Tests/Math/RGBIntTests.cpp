...
namespace Math
{
	TEST_CLASS(RGBIntTests)
	{
		...
		{
			Assert::IsTrue(std::is_same_v<std::uint8_t, PonyBase::Math::RGBInt<std::uint8_t>::ValueType>);
		}

		TEST_METHOD(StaticDataTest)
		{
			Assert::AreEqual(std::numeric_limits<std::uint8_t>::max(), PonyBase::Math::RGBInt<std::uint8_t>::MaxValue);
			Assert::AreEqual(std::size_t{3}, PonyBase::Math::RGBInt<std::uint8_t>::ComponentCount);
		}

		TEST_METHOD(PredefinedTest)
		{
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Red == PonyBase::Math::RGBInt<std::uint8_t>(255, 0, 0));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Green == PonyBase::Math::RGBInt<std::uint8_t>(0, 255, 0));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Blue == PonyBase::Math::RGBInt<std::uint8_t>(0, 0, 255));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Black == PonyBase::Math::RGBInt<std::uint8_t>(0, 0, 0));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::White == PonyBase::Math::RGBInt<std::uint8_t>(255, 255, 255));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Yellow == PonyBase::Math::RGBInt<std::uint8_t>(255, 255, 0));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Magenta == PonyBase::Math::RGBInt<std::uint8_t>(255, 0, 255));
			Assert::IsTrue(PonyBase::Math::RGBInt<std::uint8_t>::Predefined::Cyan == PonyBase::Math::RGBInt<std::uint8_t>(0, 255, 255));
		}

		TEST_METHOD(ConstructorTest)
		{
			auto defaultColor = PonyBase::Math::RGBInt<std::uint8_t>();
			...
			auto color = PonyBase::Math::RGBInt<std::uint8_t>(r, g, b);
			Assert::AreEqual(r, color.R());
			Assert::AreEqual(g, color.G());
			Assert::AreEqual(b, color.B());

			auto array = std::array{r, g, b};
			const auto arrayColor = PonyBase::Math::RGBInt<std::uint8_t>(array);
			Assert::AreEqual(r, arrayColor.R());
			Assert::AreEqual(g, arrayColor.G());
			Assert::AreEqual(b, arrayColor.B());

			constexpr auto vector = PonyBase::Math::Vector3<std::uint8_t>(r, g, b);
			auto vectorColor = PonyBase::Math::RGBInt<std::uint8_t>(vector);
		...
		TEST_METHOD(AccessTest)
		{
			...
			Assert::AreEqual(g, color.G());
			Assert::AreEqual(b, color.B());
			constexpr auto cColor = PonyBase::Math::RGBInt<std::uint8_t>(r, g, b);
		...
		TEST_METHOD(IsBlackTest)
		{
			...
			constexpr std::uint8_t g = 200;
			constexpr std::uint8_t b = 1;
			constexpr auto color = PonyBase::Math::RGBInt<std::uint8_t>(r, g, b);
		...
		TEST_METHOD(SetTest)
		{
			...
			constexpr std::uint8_t g = 200;
			constexpr std::uint8_t b = 1;
			auto color = PonyBase::Math::RGBInt<std::uint8_t>();
			color.Set(r, g, b);
			Assert::AreEqual(r, color.R());
			Assert::AreEqual(g, color.G());
			Assert::AreEqual(b, color.B());
			color = PonyBase::Math::RGBInt<std::uint8_t>();
		...
		TEST_METHOD(MinCombinedTest)
		{
			...
			constexpr auto color1 = PonyBase::Math::RGBInt<std::uint8_t>(r1, g1, b1);
		...
		TEST_METHOD(ClampTest)
		{
			...
			constexpr auto color2 = PonyBase::Math::RGBInt<std::uint8_t>(r2, g2, b2);
		...
		TEST_METHOD(ToVectorTest)
		{
			...
			constexpr std::uint8_t b = 1;
			constexpr auto color = PonyBase::Math::RGBInt<std::uint8_t>(r, g, b);
			const auto vector = static_cast<PonyBase::Math::Vector3<std::uint8_t>>(color);
		...
		TEST_METHOD(AssignmentOperatorsTest)
		{
			...
			auto otherColor = PonyBase::Math::RGBInt<std::uint8_t>(otherR, otherG, otherB);
			PonyBase::Math::RGBInt<std::uint8_t>& result = color = otherColor;
			...
			color = PonyBase::Math::RGBInt<std::uint8_t>(r, g, b);
			PonyBase::Math::RGBInt<std::uint8_t>& otherResult = otherColor = std::move(color);
		...
		TEST_METHOD(EqualTest)
		{
			...
			for (std::size_t i = 0; i < PonyBase::Math::RGBInt<std::uint8_t>::ComponentCount; ++i)
			{
		...
		static constexpr PonyBase::Math::RGBInt<std::uint8_t> ColorConstexpr()
		{
			auto color = PonyBase::Math::RGBInt<std::uint8_t>(3, 250, 128);
			auto copiedColor = color;
			auto movedColor = std::move(color);
		...
		{
			[[maybe_unused]] constexpr auto defaultColor = PonyBase::Math::RGBInt<std::uint8_t>();
			[[maybe_unused]] constexpr auto color = PonyBase::Math::RGBInt<std::uint8_t>(3, 250, 128);
			[[maybe_unused]] constexpr auto arrayColor = PonyBase::Math::RGBInt<std::uint8_t>(std::array{color.R(), color.G(), color.B()});
