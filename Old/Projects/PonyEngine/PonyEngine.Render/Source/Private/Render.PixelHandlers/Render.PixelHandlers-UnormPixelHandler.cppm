...
export module PonyEngine.Render.PixelHandlers:UnormPixelHandler;
...
{
	/// @brief Pixel handler that supports texture formats with unorm values.
	...
	class UnormPixelHandler final : public IPixelHandler
	{
	public:
		...
		UnormPixelHandler() noexcept = default;
		UnormPixelHandler(const UnormPixelHandler&) = delete;
		UnormPixelHandler(UnormPixelHandler&&) = delete;
		...
		UnormPixelHandler& operator =(const UnormPixelHandler&) = delete;
		UnormPixelHandler& operator =(UnormPixelHandler&&) = delete;
	};
}
...
namespace PonyEngine::Render::PixelHandlers
{
	template <std::uint64_t RedOffset, std::uint64_t GreenOffset, std::uint64_t BlueOffset, std::uint64_t AlphaOffset,
		std::uint64_t RedBits, std::uint64_t GreenBits, std::uint64_t BlueBits, std::uint64_t AlphaBits>
	PonyMath::Color::RGBA<float> UnormPixelHandler<RedOffset, GreenOffset, BlueOffset, AlphaOffset,
		...
		const PonyMath::Core::Vector3<std::uint32_t>& pixelCoordinate) const
	{
		...
		{
			color.R() = UnormToFloat<RedOffset, RedBits>(span);
		}
		if constexpr (GreenBits > 0)
		{
			color.G() = UnormToFloat<GreenOffset, GreenBits>(span);
		}
		if constexpr (BlueBits > 0)
		{
			color.B() = UnormToFloat<BlueOffset, BlueBits>(span);
		}
		if constexpr (AlphaBits > 0)
		{
			color.A() = UnormToFloat<AlphaOffset, AlphaBits>(span);
		}
	...
	template <std::uint64_t RedOffset, std::uint64_t GreenOffset, std::uint64_t BlueOffset, std::uint64_t AlphaOffset,
		std::uint64_t RedBits, std::uint64_t GreenBits, std::uint64_t BlueBits, std::uint64_t AlphaBits>
	void UnormPixelHandler<RedOffset, GreenOffset, BlueOffset, AlphaOffset,
		...
		const PonyMath::Core::Vector3<std::uint32_t>& pixelCoordinate, const PonyMath::Color::RGBA<float>& color) const
	{
		...
		if constexpr (RedBits > 0)
		{
			FloatToUnorm<RedOffset, RedBits>(span, color.R());
		}
		if constexpr (GreenBits > 0)
		{
			FloatToUnorm<GreenOffset, GreenBits>(span, color.G());
		}
		if constexpr (BlueBits > 0)
		{
			FloatToUnorm<BlueOffset, BlueBits>(span, color.B());
		}
		if constexpr (AlphaBits > 0)
		{
			FloatToUnorm<AlphaOffset, AlphaBits>(span, color.A());
		}
	}
}
