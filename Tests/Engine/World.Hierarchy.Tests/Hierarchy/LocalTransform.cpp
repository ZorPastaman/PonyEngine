...
{
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform2D<float>::ValueType, float>);
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform2D<float>::PositionType, PonyEngine::Math::Vector2<float>>);
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform2D<float>::RotationType, float>);
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform2D<float>::ScaleType, PonyEngine::Math::Vector2<float>>);
}

TEST_CASE("LocalTransform2D default constructor", "[World][Hierarchy][LocalTransform]")
{
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>();
...
TEST_CASE("LocalTransform2D main constructor", "[World][Hierarchy][LocalTransform]")
{
	...
	auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, rotation, scale);
	...
	transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, bigRotation, scale);
	REQUIRE(transform.Position() == position);
	...
	transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, negativeRotation, scale);
	REQUIRE(transform.Position() == position);
	...
	BENCHMARK("Bench")
	{
		return PonyEngine::World::Hierarchy::LocalTransform2D<float>(PonyEngine::Math::Vector2<float>(4.f, -2.f), -0.5f, PonyEngine::Math::Vector2<float>(-2.f, 3.f));
	};
#endif
}
...
TEST_CASE("LocalTransform2D copy constructor", "[World][Hierarchy][LocalTransform]")
{
	...
	constexpr auto scale = PonyEngine::Math::Vector2<float>(-2.f, 3.f);
	auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, rotation, scale);
	PonyEngine::World::Hierarchy::LocalTransform2D<float> copy = transform;
...
TEST_CASE("LocalTransform2D move constructor", "[World][Hierarchy][LocalTransform]")
{
	...
	constexpr auto scale = PonyEngine::Math::Vector2<float>(-2.f, 3.f);
	auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, rotation, scale);
	PonyEngine::World::Hierarchy::LocalTransform2D<float> copy = std::move(transform);
...
{
	REQUIRE(PonyEngine::World::Hierarchy::LocalTransform2D<float>::Identity() == PonyEngine::World::Hierarchy::LocalTransform2D<float>());
}
...
TEST_CASE("LocalTransform2D rotation", "[World][Hierarchy][LocalTransform]")
{
	...
	BENCHMARK("Set")
	{
		auto trans = PonyEngine::World::Hierarchy::LocalTransform2D<float>();
		trans.Rotation(15.f);
		return trans;
	};
#endif
...
TEST_CASE("LocalTransform2D trs", "[World][Hierarchy][LocalTransform]")
{
	constexpr auto position = PonyEngine::Math::Vector2<float>(4.f, -2.f);
	constexpr float rotation = 0.4f;
	constexpr auto scale = PonyEngine::Math::Vector2<float>(-2.f, 3.f);
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, rotation, scale);
...
TEST_CASE("LocalTransform2D isFinite", "[World][Hierarchy][LocalTransform]")
{
	...
	BENCHMARK("Bench")
	{
		return PonyEngine::World::Hierarchy::LocalTransform2D<float>(PonyEngine::Math::Vector2<float>(4.f, -2.f), 0.4f, PonyEngine::Math::Vector2<float>(-2.f, 3.f)).IsFinite();
	};
#endif
}
...
TEST_CASE("LocalTransform2D cast", "[World][Hierarchy][LocalTransform]")
{
	constexpr auto position = PonyEngine::Math::Vector2<float>(4.f, -2.f);
	constexpr float rotation = 0.4f;
	constexpr auto scale = PonyEngine::Math::Vector2<float>(-2.f, 3.f);
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, rotation, scale);
	const auto cast = static_cast<PonyEngine::World::Hierarchy::LocalTransform2D<double>>(transform);
	REQUIRE(PonyEngine::World::Hierarchy::AreAlmostEqual(PonyEngine::World::Hierarchy::LocalTransform2D<double>(static_cast<PonyEngine::Math::Vector2<double>>(position), rotation, static_cast<PonyEngine::Math::Vector2<double>>(scale)), cast));
}
...
TEST_CASE("LocalTransform2D copy-assignment", "[World][Hierarchy][LocalTransform]")
{
	...
	constexpr auto scale = PonyEngine::Math::Vector2<float>(-2.f, 3.f);
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform2D<float>(position, rotation, scale);
	auto copy = PonyEngine::World::Hierarchy::LocalTransform2D<float>();
	copy = transform;
	REQUIRE(copy.Position() == position);
	REQUIRE(PonyEngine::Math::AreAlmostEqual(copy.Rotation(), rotation));
	REQUIRE(copy.Scale() == scale);
...
TEST_CASE("LocalTransform2D are almost equal", "[World][Hierarchy][LocalTransform]")
{
	...
	for (std::size_t i = 0; i < 2; ++i)
	{
		...
		transform.Translate(pos);
		REQUIRE(!PonyEngine::World::Hierarchy::AreAlmostEqual(transform, copy));
		REQUIRE(PonyEngine::World::Hierarchy::AreAlmostEqual(transform, copy, PonyEngine::Math::Tolerance{.absolute = 10.f}));
		transform.Position(position);
	}
...
{
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform3D<float>::ValueType, float>);
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform3D<float>::PositionType, PonyEngine::Math::Vector3<float>>);
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform3D<float>::RotationType, PonyEngine::Math::Quaternion<float>>);
	STATIC_REQUIRE(std::is_same_v<PonyEngine::World::Hierarchy::LocalTransform3D<float>::ScaleType, PonyEngine::Math::Vector3<float>>);
}

TEST_CASE("LocalTransform3D default constructor", "[World][Hierarchy][LocalTransform]")
{
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>();
...
TEST_CASE("LocalTransform3D main constructor", "[World][Hierarchy][LocalTransform]")
{
	...
	auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, rotation, scale);
	REQUIRE(transform.Position() == position);
	REQUIRE(PonyEngine::Math::AreAlmostEqual(transform.Rotation(), rotation));
	REQUIRE(transform.Scale() == scale);

	constexpr auto bigRotation = PonyEngine::Math::Quaternion<float>(4.f, -3.f, 5.f, -1.f);
	transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, bigRotation, scale);
	REQUIRE(transform.Position() == position);
	...
	BENCHMARK("Bench")
	{
		return PonyEngine::World::Hierarchy::LocalTransform3D<float>(PonyEngine::Math::Vector3<float>(4.f, -2.f, 2.f), PonyEngine::Math::Quaternion<float>(4.f, -3.f, 5.f, -1.f), PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f));
	};
#endif
}
...
TEST_CASE("LocalTransform3D copy constructor", "[World][Hierarchy][LocalTransform]")
{
	...
	constexpr auto scale = PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f);
	auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, rotation, scale);
	PonyEngine::World::Hierarchy::LocalTransform3D<float> copy = transform;
	REQUIRE(copy.Position() == transform.Position());
	REQUIRE(copy.Rotation() == transform.Rotation());
	REQUIRE(copy.Scale() == transform.Scale());
}
...
TEST_CASE("LocalTransform3D move constructor", "[World][Hierarchy][LocalTransform]")
{
	...
	constexpr auto scale = PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f);
	auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, rotation, scale);
	PonyEngine::World::Hierarchy::LocalTransform3D<float> copy = std::move(transform);
	REQUIRE(copy.Position() == position);
	REQUIRE(PonyEngine::Math::AreAlmostEqual(copy.Rotation(), rotation));
	REQUIRE(copy.Scale() == scale);
}
...
{
	REQUIRE(PonyEngine::World::Hierarchy::LocalTransform3D<float>::Identity() == PonyEngine::World::Hierarchy::LocalTransform3D<float>());
}
...
TEST_CASE("LocalTransform3D rotation", "[World][Hierarchy][LocalTransform]")
{
	...
	BENCHMARK("Set")
	{
		auto trans = PonyEngine::World::Hierarchy::LocalTransform3D<float>();
...
TEST_CASE("LocalTransform3D trs", "[World][Hierarchy][LocalTransform]")
{
	constexpr auto position = PonyEngine::Math::Vector3<float>(4.f, -2.f, 2.f);
	const auto rotation = PonyEngine::Math::RotationQuaternion(PonyEngine::Math::Vector3<float>(-1.f, 0.5f, 2.7f));
	constexpr auto scale = PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f);
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, rotation, scale);
	REQUIRE(PonyEngine::Math::AreAlmostEqual(transform.TRSMatrix(), PonyEngine::Math::TRSMatrix(position, rotation, scale)));
	REQUIRE(PonyEngine::Math::AreAlmostEqual(transform.TRSMatrixCompact(), PonyEngine::Math::TRSMatrixCompact(position, rotation, scale)));
}
...
TEST_CASE("LocalTransform3D isFinite", "[World][Hierarchy][LocalTransform]")
{
	...
	BENCHMARK("Bench")
	{
		return PonyEngine::World::Hierarchy::LocalTransform3D<float>(PonyEngine::Math::Vector3<float>(4.f, -2.f, 2.f), PonyEngine::Math::Quaternion<float>(4.f, 6.f, -2.f, -3.f), PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f)).IsFinite();
	};
#endif
}
...
TEST_CASE("LocalTransform3D cast", "[World][Hierarchy][LocalTransform]")
{
	constexpr auto position = PonyEngine::Math::Vector3<float>(4.f, -2.f, 2.f);
	const auto rotation = PonyEngine::Math::RotationQuaternion(PonyEngine::Math::Vector3<float>(-1.f, 0.5f, 2.7f));
	constexpr auto scale = PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f);
	const auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, rotation, scale);
	const auto cast = static_cast<PonyEngine::World::Hierarchy::LocalTransform3D<double>>(transform);
	REQUIRE(PonyEngine::World::Hierarchy::AreAlmostEqual(PonyEngine::World::Hierarchy::LocalTransform3D<double>(static_cast<PonyEngine::Math::Vector3<double>>(position), static_cast<PonyEngine::Math::Quaternion<double>>(rotation), static_cast<PonyEngine::Math::Vector3<double>>(scale)), cast));
}
...
TEST_CASE("LocalTransform3D copy-assignment", "[World][Hierarchy][LocalTransform]")
{
	...
	constexpr auto scale = PonyEngine::Math::Vector3<float>(-2.f, 3.f, 0.5f);
	auto transform = PonyEngine::World::Hierarchy::LocalTransform3D<float>(position, rotation, scale);
	auto copy = PonyEngine::World::Hierarchy::LocalTransform3D<float>();
	copy = transform;
	REQUIRE(copy.Position() == position);
	REQUIRE(PonyEngine::Math::AreAlmostEqual(copy.Rotation(), rotation));
	REQUIRE(copy.Scale() == scale);
