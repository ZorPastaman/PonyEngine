...
namespace PonyEngine::Input
{
	MouseAxisMap::MouseAxisMap(IRawInputContext& input)
	{
		...
		{
			buttonAxes[i] = input.Hash(Input::Axis(std::format("{}/{}", MouseLayout::ButtonPath, i + 1)));
		}

		wheelAxes[static_cast<std::size_t>(MouseWheel::Horizontal)] = input.Hash(Input::Axis(MouseLayout::WheelHorizontalPath));
		wheelAxes[static_cast<std::size_t>(MouseWheel::Vertical)] = input.Hash(Input::Axis(MouseLayout::WheelVerticalPath));

		pointerAxes[static_cast<std::size_t>(MousePointer::X)] = input.Hash(Input::Axis(MouseLayout::AxisXPath));
		pointerAxes[static_cast<std::size_t>(MousePointer::Y)] = input.Hash(Input::Axis(MouseLayout::AxisYPath));
	}
