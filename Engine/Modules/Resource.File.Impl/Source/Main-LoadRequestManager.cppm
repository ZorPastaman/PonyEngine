...
export module PonyEngine.Resource.File.Impl:LoadRequestManager;
...
{
	std::shared_ptr<LoadRequest> LoadRequestManager::CreateRequest(const PonyEngine::File::IFile& file, const LoadParams& loadParams, 
		const std::function<void(const ILoadRequest&)>& callback) const
	{
		auto loadRequest = std::allocate_shared<LoadRequest>(requestAllocator, loadParams, callback);
		const auto readParams = PonyEngine::File::ReadParams
		{
