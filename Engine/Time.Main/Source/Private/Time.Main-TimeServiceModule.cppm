...
export module PonyEngine.Time.Main:TimeServiceModule;
...
namespace PonyEngine::Time
{
	void TimeServiceModule::StartUp(Application::IModuleContext& context)
	{
		...
		{
			const auto timeService = std::make_shared<TimeService>();
			Application::ServiceData data;
			data.SetService(timeService, PONY_ENGINE_TIME_MAIN_TICK_ORDER);
			data.AddInterface(&timeService->PublicTimeService());
