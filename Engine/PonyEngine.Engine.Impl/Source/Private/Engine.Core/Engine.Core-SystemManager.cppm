...
export module PonyEngine.Engine.Core:SystemManager;
...
namespace PonyEngine::Engine
{
	...
	void SystemManager::Initialize(const std::span<ISystemFactory*> systemFactories)
	{
		...
		std::size_t tickableCount = 0, interfaceCount = 0;
		for (ISystemFactory* const factory : systemFactories)
		{
			try
			{
				...
				std::visit([&]<typename SystemPtr>(const SystemPtr& systemPtr) noexcept
				{
					...
					if constexpr (std::is_same_v<std::shared_ptr<ITickableSystem>, SystemPtr>)
					{
						++tickableCount;
					}
				}, system.system);
		...
		tickableSystems.reserve(tickableCount);
